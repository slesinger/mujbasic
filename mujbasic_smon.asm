...
.const PCHSAVE         = $02A8         // PC hi
.const PCLSAVE         = $02A9         // PC lo
.const SRSAVE          = $02AA         // SR
.const AKSAVE          = $02AB         // A
.const XRSAVE          = $02AC         // XR
.const YSAVE           = $02AD         // YR
.const SPSAVE          = $02AE         // SP
.const IRQ_LO          = $0314         // Vector: Hardware IRQ Interrupt Address Lo
.const IRQ_HI          = $0315         // Vector: Hardware IRQ Interrupt Address Hi
.const BRK_LO          = $0316         // Vector: BRK Lo
.const BRK_HI          = $0317         // Vector: BRK Hi
.const CHRIN           = $FFCF         // Kernal input routine
.const CHROUT          = $FFD2         // Kernal output routine
.const STOP            = $FFE1         // Kernal test STOP routine
.const GETIN           = $FFE4         // Kernal get input routine
.const BRK_VECTOR    = $FFFE     // Break vector address
...
.break
        ...
        // help message
HLPMSG: .text   "A xxxx - Assemble starting at x (end assembly with 'f', use Mxx for label)"
        .byte   0
        .text   "C xxxx yyyy zzzz aaaa bbbb - Convert (execute V followed by W)"
        .byte   0
        .text   "D xxxx (yyyy) - Disassemble from x (to y)"
        .byte   0
        .text   "F aa bb cc ..., xxxxx yyyyy - Find byte sequence a b c in x-y"
        .byte   0
        .text   "FAaaaa, xxxx yyyy - Find absolute address used in opcode"
        .byte   0
        .text   "FRaaaa, xxxx yyyy - Find relative address used in opcode"
        .byte   0
        .text   "FTxxxx yyyy - Find table (non-opcode bytes) in x-y"
        .byte   0
        .text   "FZaa, xxxx yyyy - Find zero-page address used in opcode"
        .byte   0
        .text   "FIaa, xxxx yyyy - Find immediate argument used in opcode"
        .byte   0
        .text   "G (xxxx) - Run from x (or current PC)"
        .byte   0
        .text   "K xxxx (yyyy) - Dump memory from x (to y) as ASCII"
        .byte   0
        .text   "L - Load Intel HEX data from terminal"
        .byte   0
        .text   "M xxxx (yyyy) - Dump memory from x (to y) as HEX"
        .byte   0
        .text   "MS - Check and print memory size"
        .byte   0
        .text   "MT xxxx yyyy (nn) - Test memory x-y (repeat n times)"
        .byte   0
        .text   "O xxxx yyyy aa - Fill memory x-y with a"
        .byte   0
        .text   "V xxxx yyyy zzzz aaaa bbbb - Within a-b, convert addresses referencing x-y to z"
        .byte   0
        .text   "W xxxx yyyy zzzz - Copy memory x-y to z"
        .byte   0
        .text   "= xxxx yyyy - compare memory starting at x to memory starting at y"
        .byte   0
        .text   "#ddd - convert DEC to HEX and BIN"
        .byte   0
        .text   "$xx - convert HEX to DEC and BIN"
        .byte   0
        .text   "%bbbbbbbb - convert BIN to DEC and HEX"
        .byte   0
        
        // commands
ICMD:   .text "'#$%,://=?ACDFGHKLMORTVW"
...
REGHDR: .byte   $0D,$0D
        .text   "  PC  SR AC XR YR SP  NV-BDIZC"
        .byte   $00
LC0AD:  .byte   $02,$04,$01
LC0B0:  .byte   $2C,$00,$2C
LC0B3:  .byte   $59,$29,$58
...
OPMN1:   .text   "RLARESSOLLLCAASSIDCCBJJBBBBBBBBSBCCCCDDIINPPPPRRTTTTTTSS"
OPMN2:   .text   "OSSOOTBRDDDMNDTTNEPPIMSCCEMNPVVERLLLLEENNOHHLLTTAASXXYEE"
OPMN3:   .text   "LRLRRACAYXAPDCYXCCXYTPRCSQIELCSIKCDIVXYXYPAPAPISXYXASACD"
...
// ...existing code...
        // UART/VIA step tracing code removed
...
LCCE9:  bmi     LCCFD
        ...
        ldx     SPSAVE
        txs
        // Removed RTSCMD step tracing
        jmp     LCDBA
LCCFD:  // jsr     LCC65 (removed)
        lda     #$A8
        sta     $FB
        lda     #$02
        sta     $FC
...
LCD60:  sta     $02BC
        ...
        lda     $02BC
        beq     LCDA9
LCD72:  // UART/VIA step tracing code removed
LCDA9:  ldx     SPSAVE          // (4)
        txs                     // (2)
        cli                     // (2)
        lda     $02BB           // (4)
...
.const LINEBUF         = $0400         // line ("screen") buffer memory start
.const LASTCOL         = $C8           // last cursor column for input
.const NUMCOLS         = 80            // number of columns per row
.const NUMROWS         = 24            // number of rows
.const INPUT_UCASE     = 0             // do not automatically convert input to uppercase
.const TMPBUF          = $9E           // temporary storage
.const TERMCOL         = $01           // cursor column on terminal
.const LINEPTR         = $D1           // pointer to screen buffer for current line
...
PRLINE: lda     #0              // set terminal cursor position to 0
